...
| SRUM (`SRUDB.dat`) | SrumECmd | network use and connections, app resource use, push notifications, energy, vfuprov and app timeline, with user and Wi-Fi names from the SOFTWARE hive of the same volume |
| Windows Timeline (`ActivitiesCache.db`) | WxTCmd | Activity, ActivityOperations and Activity_PackageIDs, known-folder GUIDs named, durations; its `-wal` applied |
