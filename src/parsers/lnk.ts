 ...
 */
function uuidV1Time(b: Uint8Array): Date | null {
...
function macFromDroid(g: string): string | null {
...
export const lnk: Parser = {
  ...
  async *parse(reader: Reader, ctx: Ctx): AsyncGenerator<Row> {
    ...
    if (buf.length < HEADER_SIZE) {
      ctx.warn(0, `truncated header (${buf.length}/${HEADER_SIZE} bytes)`);
      return;
    }

    const c = new Cursor(buf, 0);
    if (c.u32() !== 0x4c) {
      ctx.warn(0, 'invalid HeaderSize, expected 0x4C');
      return;
    }
    ...
    if (has(0x01)) {
      if (pos + 2 > buf.length) {
        ctx.warn(pos, 'truncated target ID list size');
      } else {
        ...
        if (listStart + listSize > buf.length) {
          ctx.warn(pos, 'target ID list runs past the end of the file; reading what is there');
        }
        ...
        while (p + 2 <= listEnd) {
          ...
          if (size < 3 || p + size > listEnd) {
            ctx.warn(p, `shell item size ${size} does not fit the list; stopping here`);
            break;
          }
          items.push(parseShellItem(buf.subarray(p, p + size), p, GUID_NAMES));
          p += size;
        }
    ...
    if (has(0x02)) {
      if (pos + 4 > buf.length) {
        ctx.warn(pos, 'truncated link info');
      } else {
        ...
        if (linkInfoSize < 28 || end > buf.length) {
          ctx.warn(pos, `link info size ${linkInfoSize} does not fit the file`);
        } else {
          ...
          if ((flags & 0x01) !== 0 && volumeIdOffset > 0 && pos + volumeIdOffset + 16 <= end) {
            ...
            driveType = DRIVE_TYPES[dt] ?? `DriveUnknown (${dt})`;
            volumeSerialNumber = (vv.getUint32(8, true) >>> 0).toString(16).toUpperCase();
    ...
    const readString = (): string | null => {
      ...
      if (pos + byteCount > buf.length) {
        ctx.warn(pos, `truncated string data (need ${byteCount}, have ${buf.length - pos})`);
        pos = buf.length;
        return null;
      }
    ...
    while (pos + 8 <= buf.length) {
      ...
      if (pos + blockSize > buf.length) {
        ctx.warn(pos, `extra data block of ${blockSize} bytes runs past the end of the file`);
        break;
      }
      ...
      switch (signature) {
        ...
        case 0xa000000b: {
          ...
          if (blockSize >= 28) {
            ...
            const { GUID_NAMES } = await import('./lnk/guids');
            detail = `${GUID_NAMES[g] ?? `Unmapped GUID: ${g}`} (${g})`;
          }
          break;
        }
        case 0xa0000002: {
    ...
    // LECmd takes the target's MFT reference from the last shell item that has
    // one, which is the item naming the target itself.
    const lastWithRef = [...items].reverse().find((i) => i.mftEntry !== null);
    ...
    const common = {
      ...
      iconLocation,
      description,
      fileAttributes: flagNames(fileAttributes, FILE_ATTRIBUTES),
      headerFlags: flagNames(linkFlags, DATA_FLAGS),
      driveType,
      volumeSerialNumber,
      volumeLabel,
      localPath,
      networkPath,
      commonPath,
      targetIdAbsolutePath: items.length > 0 ? absolutePath(items) : null,
      machineId,
      machineMacAddress,
      trackerCreatedOn,
      extraBlocksPresent: blocks.length > 0 ? blocks.map((b) => b.name).join(', ') : null,
    ...
    yield {
      ...
      entryType: 'Target',
      path:
        (localPath ?? '') + (commonPath ?? '') ||
        networkPath ||
        (items.length > 0 ? absolutePath(items) : null),
      itemKind: 'Shortcut',
      shortName: null,
      targetCreated,
      targetModified,
      targetAccessed,
      fileSize,
      mftEntry: lastWithRef?.mftEntry ?? null,
      mftSequence: lastWithRef?.mftSequence ?? null,
      fileSystemHint: lastWithRef?.fileSystemHint ?? null,
      offset: 0,
    };
    ...
    for (const block of blocks) {
      yield {
        ...
        itemKind: block.name,
        shortName: null,
        targetCreated: block.signature === 0xa0000003 ? trackerCreatedOn : null,
        targetModified: null,
        targetAccessed: null,
        fileSize: block.size,
        mftEntry: null,
